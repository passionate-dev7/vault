...
Metric plan: 20 titles measured, 17 failing, worst 14.2 LU from the EBU R128 target of -23.0 LUFS. Checked at `/api/stats` and `/api/catalog` against `vault.fleet`, and re-checked independently with `ffmpeg -af ebur128` against the public source URL stored in `vault.sources` and printed next to every measurement.

Live by: 2026-09-08, live now at https://vault-387894104564.us-central1.run.app on Cloud Run against ClickHouse Cloud, ahead of the 2026-09-09 deadline. Deploys are the coordinator's; this workstream ships to the working tree.
