...
### Five slots this product owns

| Slot | Redslip owns |
|---|---|
| Incident | Monday at an archive digitization desk: twenty masters sitting in a drop folder, and no page that ranks which ones a streamer would reject today |
| Named user | Jordan, archive digitization / catalog delivery lead who allocates mix-stage hours across a fleet, not the operator closing one outbound file |
| Named incumbent | Telestream Vantage, Venera Pulsar, Interra Baton: excellent per-file report stacks that never answer "which of these do I touch first this week" |
| Measured number | 11 BATCH / 6 HUMAN / 3 READY across 20 titles; ranking reads 320 rollup rows in 16.5 ms while the forensic stream behind them holds 24,017 ClickHouse samples |
| Honest limit | Redslip measures, ranks and dispatches. It does not repair. A BATCH lane is a work order, not a rewritten WAV |

That is a different product from a single-master measure-repair-remeasure loop. Same
ClickHouse track, opposite verb: triage a corpus, do not close one file.

### How the page is built

The document below follows the product: the catalog first, then one row out of it, then
the raw measurement underneath that row. Every claim carries the statement that produced
it and what the statement cost to run.
...
Night Tide's own slip prints its source command from `vault.sources`. The worst title
in the ledger is Fugitive Valley, and that is the one a stranger can check without
cloning anything:
...
**Incident.** An archive digitization lead opens the week with a drop folder of masters
and a mix-stage calendar that only has hours for a handful of them. Rejections, when they
come, arrive as separate emails to separate people, weeks apart. Nobody ever sees the
fleet as one ranked list, so the same class of failure burns the same hours over and over.

**Named user.** That digitization / catalog delivery lead. Their job is allocation: which
titles burn mix-stage time this week, which ones a gain pass can clear unattended, which
ones already ship. They are not the person sitting on one outbound file with a loudness
meter.

**Named incumbent.** Telestream Vantage, Venera Pulsar and Interra Baton. Each answers
"is this file good" with a thick per-file report. None of them answer "of these two
hundred, which twelve do I touch first, and which of those twelve can a machine take".
A stack of PDFs is not a ranked work queue.

**Concrete case anyone can open.** `NightTide_512kb.mp4` on archive.org measures at
`I: -17.4 LUFS` over its first 120 seconds. EBU R128 wants -23.0 LUFS with 1.0 LU of
tolerance, so that master is 5.6 LU out. Fugitive Valley sits 14.2 LU off. Those are
inputs to a ranking, not the product. The product is the ordered fleet and the lane on
each row.

Measuring one master was never the hard part. `ebur128` has told you a title sits 14 LU
off target for years, for free. The hard part is holding a catalog of those measurements
so the ordering question stays cheap while the forensic question stays possible. Ordering
wants one small row per title. Forensics want every 100 ms reading ffmpeg ever emitted.
That tension is what a column store is for, and it is why Redslip's ranking query never
touches `vault.loudness_samples`.
...
title goes to. It stops there. The slip is a work order. The WAV is untouched.
...
The hard product limit first: Redslip does not rewrite audio or subtitles. BATCH means
"safe for an unattended gain pass somewhere else"; it is not a repaired file inside this
service. A sibling product on the same track owns measure, repair and re-measure of one
master. Redslip owns the fleet question that product cannot answer.

Five further questions have an obvious machine answer that would be wrong, so Redslip
does not answer them either.
...
that ships SRTs, so the TTSS path earns findings rather than only tests. Handing a BATCH-lane title to a `loudnorm` pass stays out of scope on purpose: that closed
repair loop is Checkprint's verb on this track, not Redslip's. Redslip measures, ranks and
dispatches, and the slip says so.
